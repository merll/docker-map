...
class AbstractRunner(with_metaclass(RunnerMeta, PolicyUtil)):
    ...
    def run_actions(self, action_list):
        ...
        :param action_list: List of actions to apply.
        :type action_list: list[dockermap.map.action.ClientMapActions]
        ...
        for cma in action_list:
            client_config = policy.clients[cma.client_name]
            client = client_config.get_client()
            c_map = policy.container_maps[cma.map_name]

            for action in cma.actions:
                if action.config_type == ITEM_TYPE_CONTAINER:
                    config = c_map.get_existing(action.config_name)
                    item_name = policy.cname(cma.map_name, action.config_name, action.instance_name)
                    existing_items = policy.container_names[cma.client_name]
                    ...
                    # TODO
                    config = None
                    item_name = policy.nname(cma.map_name, action.config_name)
